 ...
 * It uses pdf-lib from a CDN and implements compression techniques to reduce file size
 * while preserving visual quality.
 * 
 * This file is placed in the public folder to avoid ESLint and bundling issues.
 */
...
async function optimizeContentStreams(pdfDoc, options) {
  ...
  // Apply different optimization levels based on compression settings
  const pages = pdfDoc.getPages();
  
  for (let i = 0; i < pages.length; i++) {
    const page = pages[i];
    
    // If high compression and not preserving quality, we can be more aggressive
    if (options.compressionLevel === 'high' && !options.preserveQuality) {
      // Remove unnecessary page resources
      const dict = pdfDoc.context.lookup(page.ref);
      
      // Remove outlines, article threads, etc.
      const keysToRemove = ['B', 'Dur', 'Trans', 'AA', 'StructParents'];
      keysToRemove.forEach(key => {
        if (dict.has(PDFLib.PDFName.of(key))) {
          dict.delete(PDFLib.PDFName.of(key));
        }
      });
    }
  }
